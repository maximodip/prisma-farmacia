...
export async function createAccount(formData: FormData): Promise<void> {
  ...
  redirect(`/customers/${customerId}/accounts`)
}

export async function removeAccount(formData: FormData) {
  'use server'
  const accountId = parseInt(formData.get('accountId')?.toString() || '0')
  ...
  // Use transaction to delete items first, then the account
  await prisma.$transaction([
    prisma.accountItem.deleteMany({
      where: { accountId },
    }),
